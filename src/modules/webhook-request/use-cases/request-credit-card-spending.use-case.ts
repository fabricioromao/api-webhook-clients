...
export class RequestCreditCardSpendingUseCase {
  constructor(
    ...
    @InjectQueue(QueuesEnum.CREDIT_CARD_SPENDING)
    private creditCardSpendingQueue: Queue,
  ) {}
  ...
  async execute(req: RequestType) {
    ...
    await this.creditCardSpendingQueue.add(
      QueuesEnum.CREDIT_CARD_SPENDING,
      new CreditCardSpendingDto({
